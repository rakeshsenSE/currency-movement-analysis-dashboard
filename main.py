...
currency_pair=st.sidebar.selectbox("Select Currency Pair:",["USDBDT=X","EURUSD=X","GBPUSD=X","AUDUSD=X","USDJPY=X","GBPJPY=X"])
...
if df.empty or 'Close' not in df.columns or len(df) == 0:
    st.error(f"No data retrieved for {currency_pair} with period {period}. Please try again or select another pair.")
else:
